    ...
    switch (move.id) {
      ...
      case "protect":
        if (!isDoubles && (counter.setupType && !moves.has("wish") || moves.has("rest")))
          return { cull: true };
        if (!isDoubles && counter.get("Status") < 2 && ["Speed Boost", "Moody"].every((m) => !abilities.has(m)))
          return { cull: true };
      ...
      case "uturn":
        ...
        return { cull: !!counter.get("speedsetup") || counter.setupType && !bugSwordsDanceCase || isDoubles && moves.has("leechlife") };
      case "explosion":
