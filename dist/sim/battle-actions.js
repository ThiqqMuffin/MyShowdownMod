...
class BattleActions {
  ...
  useMoveInner(moveOrMoveName, pokemon, target, sourceEffect, zMove, maxMove) {
    ...
    if (!move.negateSecondary && !(move.hasSheerForce && pokemon.hasAbility("sheerforce")) && !move.isFutureMove) {
  ...
  canMegaEvo(pokemon) {
    ...
    const item = pokemon.getItem();
    if ((this.battle.gen <= 7 || this.battle.ruleTable.has("standardnatdex")) && altForme?.isMega && altForme?.requiredMove && pokemon.baseMoves.includes((0, import_dex.toID)(altForme.requiredMove)) && !item.zMove) {
